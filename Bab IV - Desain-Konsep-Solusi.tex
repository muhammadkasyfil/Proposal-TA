...
\begin{itemize}
  \item \textbf{CPUID-based hypervisor detection} menggunakan instruksi \texttt{CPUID} \textit{leaf} \texttt{0x40000000} untuk mengidentifikasi signature hypervisor.
  \item \textbf{Registry scanning} memeriksa \textit{registry} untuk \textit{artifacts} khas dari VM software seperti VirtualBox, VMware, atau QEMU di jalur \textit{registry} seperti \texttt{HKLM\textbackslash SYSTEM\textbackslash CurrentControlSet\textbackslash Services} dan \texttt{HKLM\textbackslash HARDWARE\textbackslash DESCRIPTION\textbackslash System}.
  \item \textbf{Process enumeration} melakukan enumerasi terhadap \textit{running processes} untuk menemukan \textit{process names} khas dari \textit{analysis tools} populer seperti \texttt{procmon.exe}, \texttt{wireshark.exe}, \texttt{IDA.exe}, atau \texttt{x64dbg.exe}.
  \item \textbf{DLL inspection} memeriksa \textit{loaded DLLs} dalam \textit{current process} untuk menemukan \textit{monitoring} atau \textit{analysis DLLs} yang dapat mengindikasikan \textit{instrumentation tools}.
  \item \textbf{Anti-debugging checks} melakukan pemeriksaan terhadap PEB (\textit{Process Environment Block}) flags yang mengindikasikan \textit{debugger attachment}, serta \textit{checks} untuk memastikan tidak ada \textit{kernel debugger} yang \textit{active}.
\end{itemize}
